...
src/
  ...
  eval/          算法评测（切分对比、递减模型回测 eval_dca）+ 智能体评测集（单井 37 个模板 + 单元 20 个模板）
...
docs/            dev-plan-sec-unit.md · dev-plan-phase2.md · dev-plan-phase3.md —— 各阶段开发计划与取舍
web/index.html   前端：单文件、零依赖、手写 SVG
tests/           test_kernel · test_agent · test_unit · test_report_snapshot · test_phase2 · test_models_phase3，共 120 项（stdlib unittest）
```
...
| 系统 · 模型评测 | 智能体指标与达标状态、分类别表现、三种切分对比（含梯度提升与未加守卫的融合）、递减模型回测表；失败样例：预测模型误差最大的井、智能体六类问题、递减模型高估最多的井 |
...
**递减模型回测**（`eval-dca`：204 口类比井库 + 206 口回测井，只用峰后前 H 个月拟合；EUR 与合成真值比）

| 峰后历史 | 方法 | 未来 12 月产量误差 | EUR 中位误差 | EUR 中位偏差 | 高估 > 20% 的井 |
| --- | --- | ---: | ---: | ---: | ---: |
| 6 个月 | 经验 Arps | 5.0% | 11.3% | +3.7% | 22.8% |
| 6 个月 | 物理约束 · 类比先验 + EUR 上限 | 4.9% | **7.9%** | +3.9% | **11.7%** |
| 12 个月 | 经验 Arps | **1.9%** | **5.1%** | +2.0% | 21.8% |
| 12 个月 | 物理约束 · 类比先验 + EUR 上限 | 3.4% | 6.7% | +1.0% | 20.9% |
| 24 个月 | 经验 Arps | **1.1%** | **12.6%** | +12.6% | 44.9% |
| 24 个月 | 物理约束 · 类比先验 + EUR 上限 | 3.3% | 15.7% | +12.3% | 44.3% |

历史短时物理约束把"高估超过 20%"的井减半；历史长了经验 Arps 更准 —— 合成数据的递减本身按 Arps 生成，这个结果在意料之中。
官方储量口径因此仍用经验 Arps，物理约束模型作为短历史井的复核。
...
回测证明它对短历史井更稳，对长历史井不如 Arps，所以只做复核、不替换官方口径。
...
- **物理约束递减在合成数据上没有主场优势**：合成递减按 Arps 生成、没有真实的线性流段，长历史回测不如经验 Arps；它的价值要在真实致密油井上再验证。
  峰后有效月份不足 12 个时不做自助法，区间只是 ±40% 经验带；RNP 诊断用井口压力代替井底流压，只看形态。
