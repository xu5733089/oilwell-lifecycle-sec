...
| 系统 · 模型评测 | 智能体指标与达标状态、分类别表现、三种切分对比、递减模型回测表 |
...
| --- | ---: | ---: | ---: | ---: | ---: |
| 见油时间 (d) | 0.89 | 5.79 | **+84.6%** | 0.82 | 0.61 |
| 达峰时间 (d) | 29.6 | 46.9 | **+36.9%** | 0.82 | 0.61 |
| 达峰产量 (t/d) | 1.01 | 4.65 | **+78.3%** | 0.82 | 0.69 |
| 达峰压力 (MPa) | 0.50 | 2.24 | **+77.4%** | 0.77 | 0.58 |
| EUR (t，自然递减口径) | 1195 | 3603 | **+66.8%** | 0.64 | 0.36 |

训练 / 校准 / 测试 = 253 / 91 / 114 口（校准集再交错对半：45 口选融合权重、46 口做保形校准）；数据质量门禁通过 632 / 660 口。
基线 = 邻井类比 + Arps 外推。`eval-algo` 显示随机切分在 5 个目标上平均乐观 18.7%，对外一律用时间切分。

**模型族对比**（测试集 MAE，各自保形校准；融合权重 = 序列模型所占权重）

| 目标 | 梯度提升 | 序列模型 | 融合（线上） | 融合权重 |
| --- | ---: | ---: | ---: | ---: |
| 见油时间 (d) | **0.75** | 1.72 | 0.89 | 0.25 |
| 达峰时间 (d) | **29.56** | 30.37 | 29.57 | 0.75 |
| 达峰产量 (t/d) | 1.14 | 1.16 | **1.01** | 0.50 |
| 达峰压力 (MPa) | 0.51 | 0.62 | **0.51** | 0.50 |
| EUR (t) | 1367 | 1200 | **1195** | 0.25 |

EUR 融合后较单用梯度提升下降 12.5%，达峰产量下降 11%；见油时间反而变差（0.75 → 0.89）—— 权重只在 45 口井上选，样本小、会选错，如实保留。

**按区块留出时融合反而拖累**（`eval-algo` 的 group 切分）：校准集来自训练区块，序列模型在那里表现好、被选了高权重（EUR、达峰时间权重 1.0），
到没见过的区块却外推失败 —— EUR MAE 梯度提升 691、序列模型 1778、融合 1778；见油时间 0.89 / 8.09 / 6.18。
线上模型用时间外推切分，不受影响；但这说明序列模型对区块迁移不稳，换新区块使用前必须重选权重（见"已知问题"）。
...
**可解释性自检**：TreeSHAP 与穷举 Shapley 逐位一致（误差 < 1e-9），真实模型包上可加性误差 < 1e-10，150 口井 5 个目标合计约 0.5 秒；
...
内网装不上 torch，三层膨胀卷积用 numpy 手写前向、反向与 Adam，训练十秒量级，梯度有数值校验。融合不预设"一定更好"，权重由校准集选。

**为什么自己写 TreeSHAP。** 内网装不上 shap 包，而特征消融不满足可加性、画不成瀑布图。路径依赖 TreeSHAP 的多项式形式并不长，
直接读 sklearn 的树结构实现，并用穷举 Shapley 定义做逐位对拍 —— 自研实现必须有独立的正确性证据。
...
- **EUR 区间覆盖率 0.64**（名义 0.80，上一版纯梯度提升为 0.68）。时间外推切分下校准集与测试集分布漂移明显，校准集对半后样本更少；
  单井 SEC 清单的"合理确定性"一项因此如实判为"需人工确认"。下一步试按目标分组的 Mondrian 保形、或用交叉拟合保留全部校准样本。
- **融合权重样本小、且看不见区块迁移**：只在 45 口井上选，见油时间选了 0.25 反而变差；按区块留出时序列模型外推失败、融合 EUR MAE 从 691 升到 1778。
  下一步改为按区块交叉选择权重（留一个区块做权重验证），并给序列模型加输入分布漂移检测，漂移时退回梯度提升。
