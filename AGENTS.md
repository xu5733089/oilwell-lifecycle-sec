...
- **标准化计量（ASC 932）与减值测试同一剖面**：已开发部分沿用单元级指数剖面，未开发部分沿用 PUD 取值的类型曲线截断，数量必须与证实储量合计一致；
  所得税率是 `conf/config.yaml` 的示例假设（`finance.income_tax_rate`），折现率固定 10%，不许在代码里改；两期变动表的"数量变化及其他"是轧差项，不许手填。
- **概率汇总只说明保守程度，不改在账数字**：误差分布来自递减回测明细（`eval-dca` 产物），相关系数组数不足 5 个时不用估计值、只做敏感性；
  SEC 已证实储量仍按确定性低值报告。
